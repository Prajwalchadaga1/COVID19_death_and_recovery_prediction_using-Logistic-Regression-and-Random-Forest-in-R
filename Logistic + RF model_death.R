...
covid_19 = covid_19 %>% select(death,reporting.date, country, gender, age, visitingWuhan, fromWuhan, mainsymptoms, symptom)  %>% 
  mutate(
...
#Plotting Age vs Risk of Death
ggplot(covid_19, aes(death, age, fill = death))+
  geom_boxplot()+
  theme_classic()+
  scale_fill_manual(values = c('blue','red'))

#Plotting Gender vs Risk of Death
ggplot(covid_19, aes(death, fill = gender))+
  geom_bar(position ='fill')+
  theme_classic()+
  scale_fill_manual(values = c('red','blue','black'))

#Most of Deaths were in older and male sex
table1 = tableby(death ~ age + gender, total = FALSE, data = covid_19)
summary(table1, text = TRUE)



#Creating a dataset with only the essential columns used to create the logistic model
covidLR = covid_19 %>% select(death, gender, age,  visitingWuhan, fromWuhan, mainsymptoms)
...
log_model = glm(formula = death~., family = binomial(link = "logit"), data= Train)
summary(log_model)

#Predicting the death for the Test dataset and comparing it with the Test dataset to determine accuracy of the logistic model
result = predict(log_model, newdata=Test[,2:6], type="response")
result
result = ifelse(result>0.5,1,0) 
result
accuracy = mean(result == Test$death)
accuracy
...
for (i in 1:num_iterations) {
  inTrain = createDataPartition(y=covidLR$death, p=0.75, list=FALSE)
  X_train = covidLR[inTrain, ]
  X_test = covidLR[-inTrain, ]
  model = glm(formula = death~., family = binomial(link = "logit"), data= X_train)
  result = predict(model, newdata=X_test[,2:6], type="response")
  result = ifelse(result>0.5,1,0) 
  accuracy = mean(result == X_test$death)
  acc_history[[i]] = accuracy
  model_1 = glm(formula = death~visitingWuhan, family = binomial(link = "logit"), data= X_train)
  ...
  accuracy_visitingwuhan = mean(result_visitingwuhan == X_test$death)
...
covid_RF = covid_19 %>% select(death, gender, age,  visitingWuhan, fromWuhan, mainsymptoms)
...
model_RF = randomForest(death~., data=Train_RF)
model_RF

#Predicting the death for the Test dataset and comparing it with the Test dataset to determine accuracy of the RandomForest model
prediction_RF = predict(model_RF, newdata=Test_RF)
prediction_RF
accuracy_RF = mean(prediction_RF == Test_RF$death)
accuracy_RF
...
for (i in 1:num_iterations_RF) {
  inTrain_RF = createDataPartition(y=covid_RF$death, p=0.75, list=FALSE)
  X_train_RF = covid_RF[inTrain_RF, ]
  X_test_RF = covid_RF[-inTrain_RF, ]
  model_RF = randomForest(death~., data=X_train_RF)
  model1_RF = randomForest(death~visitingWuhan, data=X_train_RF)
  prediction_RF = predict(model_RF, newdata=X_test_RF)
  prediction1_RF = predict(model1_RF, newdata=X_test_RF)
  accuracy_RF = mean(prediction_RF == X_test_RF$death)
  accuracy1_RF = mean(prediction1_RF == X_test_RF$death)
